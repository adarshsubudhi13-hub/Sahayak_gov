...
drop policy if exists "Users can manage own saved schemes" on public.saved_schemes;
...
drop policy if exists "Users can insert chat logs" on public.chat_logs;
...
drop policy if exists "Users can insert flagged responses" on public.flagged_responses;
create policy "Users can insert flagged responses"
  on public.flagged_responses for insert
  with check (auth.uid() = user_id or user_id is null);

drop policy if exists "Admins can manage flagged responses" on public.flagged_responses;
...
drop trigger if exists flagged_responses_updated_at on public.flagged_responses;
...
create index if not exists idx_telemetry_scheme_state on public.telemetry_events(scheme_id, state_id);
...
create index if not exists idx_flagged_status on public.flagged_responses(status);
...
create table if not exists public.consent_records (
  ...
  channel         text default 'web' check (channel in ('web', 'whatsapp', 'ivr')),
  ip_hash         text,
  created_at      timestamptz default now()
);
...
drop policy if exists "System can insert rate limit events" on public.rate_limit_events;
...
create index if not exists idx_rate_limit_user_action on public.rate_limit_events(user_id, action, created_at);
