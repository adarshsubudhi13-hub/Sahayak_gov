...
import {
  fetchFlaggedResponses, resolveFlaggedResponse
} from '../lib/supabase/db.js';
import {
  AlertTriangle, CheckCircle2, Clock, MessageSquare,
  ShieldCheck, ChevronDown, ChevronUp, Database
} from 'lucide-react';

// ── Seed demo items for localStorage demo mode ────────────────────────────────
const DEMO_QUEUE = [
  {
    id: 'flag-demo-1',
...
export function AdminReviewQueuePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (isSupabaseConfigured) {
        const data = await fetchFlaggedResponses();
        if (data !== null) {
          if (data.length > 0) {
            setIsRealData(true);
            setQueue(data);
      ...
      // Seed demo items
      setQueue(DEMO_QUEUE);
      localStorage.setItem('sahayak_review_queue', JSON.stringify(DEMO_QUEUE));
      setLoading(false);
    }
  ...
  const handleResolve = async (itemId) => {
    const note = resolutionNotes[itemId] || '';

    if (isSupabaseConfigured && isRealData) {
      await resolveFlaggedResponse(itemId, note, userId);
    }

    // Optimistic update + localStorage sync
    const updated = queue.map(item =>
      item.id === itemId
        ? { ...item, status: 'resolved', resolution_note: note }
        : item
    );
    setQueue(updated);
    ...
    <div className="flex-1 max-w-5xl w-full mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-xs text-amber-100/90 mt-1">
              Review flagged citizen AI answers and low-confidence responses.
            </p>
          </div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
          ...
          <span className="text-xs font-bold text-amber-800">{pendingItems.length} Pending</span>
        </div>
        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2.5">
          <CheckCircle2 className="w-4 h-4 text-emerald-600" />
          <span className="text-xs font-bold text-emerald-800">{resolvedItems.length} Resolved</span>
        </div>
      </div>
      ...
      {!loading && pendingItems.length === 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center space-y-3">
          ...
          <p className="text-xs text-gray-500">No pending items in the review queue.</p>
        </div>
      )}
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-gray-500 mt-0.5">
                      {item.flag_reason === 'user_flagged' ? '🚩 Citizen flagged' : '⚠️ Low confidence'}
                      {' • '}
          ...
          {resolvedItems.map(item => (
            <div key={item.id} className="bg-gray-50 rounded-xl border border-gray-100 px-5 py-3 flex items-center justify-between gap-4 opacity-70">
              <div className="min-w-0">
                ...
                {(item.resolution_note || item.resolutionNote) && (
                  <p className="text-[11px] text-emerald-700 mt-0.5 italic">
                    Resolution: {item.resolution_note || item.resolutionNote}
                  </p>
                )}
              </div>
